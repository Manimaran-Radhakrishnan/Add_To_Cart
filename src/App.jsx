  ...
  return (
    <div className="App">
      ...
      <div className="shopCard">
        {source.map((value) => (
          <AddCard data={value} count={count} setcount={setcount} />
        ))}
      </div>
    </div>
  );
...
function AddCard({ data, count, setcount }) {
  ...
  const card_type = addCard ? "Add to Cart" : "Remove from Cart";
  ...
  return (
    <div className="addCard">
      ...
      <div className="card-priceTag">
        ...
        <p>{data.stars}</p>
        <p>
          <span className="unprice">{data.unprice} </span>
          {data.price}
        </p>
      </div>
  ...
  return (
    <div className="nevigationBar">
      ...
      <div className="nevContainer_2">
        <p className="count_btn">
          🛒 Cart <p className="count">{count}</p>
        </p>
      </div>
    </div>
  );
